<template>
	<view class="content">
		<!-- #ifdef APP-PLUS -->
			...
			<cmd-nav-bar class="nav-bar" :fixed="false" back background-color="linear-gradient(to right, #EF9435, #E95E28)" title="当日委托" font-color="#fff"></cmd-nav-bar>
		<!-- #endif -->
		<mescroll-uni :down="downOption" @down="downCallback" :up="upOption" @up="upCallback">
		<view class="tb">
			<!-- <view class="tbson thTop">
				<view>名称/代码</view>
				<view  style="width: 30%;">时间/类型</view>
				<view >价格/委托数</view>
				<view  style="width: 15%;">状态</view>
				<view  style="width: 15%;">操作</view>
			</view>
			<view class="tbson td" v-for="(el,i) in buysData" :key="i">
				<view><text>{{el.stockName}}\n</text> <text>{{el.stockCode}}</text></view>
				<view  style="width: 30%;"><text>{{el.createTime}} {{el.orderType.name}}</text></view>
				<view >{{el.orderUnitPrice}}元/{{el.orderNumber}}</view>
				<view  style="width: 15%;">
					{{el.orderStatus.name}}
				</view>
				<view  style="width: 15%;"><text style="width: 100%;" v-if="el.orderStatus.value==0" @click="cancle(el,i)">撤单</text></view>
			</view> -->
			<view style="width: 100%;" v-for="(el,i) in buysData" :key="i">
				<view style="padding: 2% 4%;width: 92%;">
					<view class="tbson td" style="height: 48upx;width: 100%;">
						...
						<view style="width: 20%;">{{el.stockCode}}</view>
						...
						<view style="width: 15%;float: left;color:#0000FF;" v-if="el.orderStatus.value==0"  @click="jumpSell(el,i)">撤单</view>
					</view>
					<view class="tbson td" style="height: 48upx;width: 100%;">
						<view style="width: 30%;height: 48upx;">{{el.orderType.name}}</view>
						<view style="width: 20%;">委托价/数</view>
					...
					<view class="tbson td" style="height: 48upx;width: 100%;">
						...
						<view style="width: 20%;">成交价/数</view>
					...
					<view class="tbson td" style="height: 48upx;width: 100%;">
						...
						<view style="width: 20%;" v-if="el.orderType.value==1&&el.orderStatus.value==1">盈利</view>
						<view style="width: 30%;text-align: right;" v-if="el.orderType.value==1&&el.orderStatus.value==1">{{el.profitprice}}</view>
					</view>
				</view>
		...
		methods: {
					...
					success: function (res) {
						if (res.confirm) {
							...
							http.get('stock/order/cancellations',{
								...
								uni.redirectTo({url: '/pages/main/transaction/todayEntrust/todayEntrust'});
								}
							})
						} else if (res.cancel) {
			...
			queryHistory(){
				...
				http.get('stock/order/getList', {
					...
					console.info(Number(_this.pageNum) + "|||" +Number(res.data.data.pages) +  "|||" + (Number(_this.pageNum) < Number(res.data.data.pages)));
