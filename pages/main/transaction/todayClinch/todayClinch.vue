<template>
	<view class="content">
		<!-- #ifdef APP-PLUS -->
			...
			<cmd-nav-bar back background-color="linear-gradient(to right, #EF9435, #E95E28)" title="当日成交" font-color="#fff"></cmd-nav-bar>
		<!-- #endif -->
		<view class="tb">
			...
			<view class="tbson td" v-for="(el,i) in buysData" :key="i">
				...
				<view>{{el.price}}元<br/>{{el.serviceFee}}元</view>
				<view>
					{{el.flag == 'S' ? '成功' : ''}}
					{{el.flag == 'F' ? '失败' : ''}}
					{{el.flag == 'C' ? '取消' : ''}}
	...
	　　},
		methods: {
			...
			queryHistory(){
				...
				http.get('transaction/findTransactionRecord',{phone:this.$store.state.userInfo.phone,starTime:this.date1,endTime:this.date2,flag:'S'}).then((res)=>{
