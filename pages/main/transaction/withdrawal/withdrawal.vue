<template>
	<view class="content">
		<!-- #ifdef APP-PLUS -->
			...
			<cmd-nav-bar class="nav-bar" :fixed="false" back background-color="linear-gradient(to right, #EF9435, #E95E28)" title="撤单" font-color="#fff"></cmd-nav-bar>
		<!-- #endif -->
		...
		<view style="width: 100%;" v-for="(el,i) in buysData" :key="i">
			<view style="padding: 2% 4%;width: 92%;">
				<view class="tbson td" style="height: 48upx;width: 100%;">
					...
					<view style="width: 15%;text-align: right;" v-else-if="el.orderStatus.value==0">{{el.orderStatus.name}}</view>
					<view style="width: 15%;float: left;color:#0000FF;" v-if="el.orderStatus.value==0" @click="cancelStock(el,i)">取消</view>
				</view>
				<view class="tbson td" style="height: 48upx;width: 100%;">
				...
				<view class="tbson td" style="height: 48upx;width: 100%;">
					...
					<view style="width: 30%;" v-if="el.profitprice!=0">盈利</view>
					<view style="width: 30%;text-align: right;" v-if="el.profitprice!=0">{{el.profitprice}}</view>
				</view>
			</view>
				...
				success: function (res) {
					if (res.confirm) {
						http.get('stock/order/cancellations',{id:el.id}).then((res)=>{
							...
							uni.redirectTo({url: '/pages/main/transaction/withdrawal/withdrawal'});
						})
