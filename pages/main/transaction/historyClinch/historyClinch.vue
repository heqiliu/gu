<template>
	<view class="content">
		...
		<cmd-nav-bar class="nav-bar" :fixed="false" back background-color="linear-gradient(to right, #EF9435, #E95E28)" title="历史成交" font-color="#fff"></cmd-nav-bar>
		<!-- #endif -->
			...
			<view style="width: 100%;" v-for="(el,i) in buysData" :key="i">
				<view style="padding: 2% 4%;width: 92%;">
					...
					<view class="tbson td" style="height: 48upx;width: 100%;">
						...
						<view style="width: 30%;" v-if="el.orderType.value!=0 && el.profitprice !=0">盈利</view>
						<view style="width: 30%;text-align: right;" v-if="el.orderType.value!=0 && el.profitprice !=0">{{el.profitprice}}</view>
					</view>
				</view>
